...
def fake_pptx(tmp_path: Path) -> Path:
    ...
    # The fallback backend uses python-pptx to open the file, but for
    # dispatch tests we mock the backend call, so a zero-byte file is enough
    # to pass validate_pptx (which only checks existence + extension).
...
def test_exporter_detects_backend() -> None:
    with mock.patch("pptx_exporter.exporter.detect_backend", return_value="fallback"):
        exp = Exporter()
...
def test_export_dispatches_to_macos_backend(fake_pptx: Path, out_dir: Path) -> None:
    with (
        mock.patch("pptx_exporter.exporter.detect_backend", return_value="macos"),
        mock.patch("pptx_exporter.exporter.detect_backend", return_value="macos"),
    ):
        exp = Exporter()
        exp.backend = "macos"
...
def test_export_dispatches_to_fallback_backend(fake_pptx: Path, out_dir: Path) -> None:
    exp = Exporter.__new__(Exporter)
    exp.backend = "fallback"
    exp.backend_label = "Fallback"

    with mock.patch(
        "pptx_exporter.platforms.fallback.export_slides"
    ) as mock_export:
        exp._export_fallback(fake_pptx, out_dir, None)
        mock_export.assert_called_once_with(fake_pptx, out_dir, progress_callback=None)
...
def test_export_calls_progress_callback(fake_pptx: Path, out_dir: Path) -> None:
    ...
    with mock.patch(
        ...
        # Simulate the platform module calling the callback twice
        ...
        exp._export_fallback(fake_pptx, out_dir, cb)
...
def test_full_export_flow(fake_pptx: Path, out_dir: Path) -> None:
    """Full Exporter.export() call with the fallback backend mocked out."""
    with mock.patch("pptx_exporter.exporter.detect_backend", return_value="fallback"):
        exp = Exporter()

    with mock.patch("pptx_exporter.platforms.fallback.export_slides") as mock_export:
